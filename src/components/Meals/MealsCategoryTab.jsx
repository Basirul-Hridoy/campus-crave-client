...
const MealsCategoryTab = () => {
    ...
    return (
        <div className="bg-slate-50 pb-24">
            ...
            {/*//*== Tab Button Start ==*/}
            <div className=" py-3 md:w-1/4 mx-auto ">
                <div className="flex justify-between md:justify-evenly px-4 md:px-8 py-2 rounded-full border my-8 bg-primary text-white">
                    <button className={` font-semibold ${activeTab === 'all' ? 'px-3 py-1 bg-secondary rounded-md whitespace-nowrap' : ''}`} onClick={() => setActiveTab('all')}>
                        All Meals
                    </button>
                    <button className={` font-semibold ${activeTab === 'breakfast' ? 'px-3 py-1 bg-secondary rounded-md whitespace-nowrap' : ''}`} onClick={() => setActiveTab('breakfast')}>
                        Breakfast
                    </button>
                    <button className={` font-semibold ${activeTab === 'lunch' ? 'px-3 py-1 bg-secondary rounded-md whitespace-nowrap' : ''}`} onClick={() => setActiveTab('lunch')}>
                        Lunch
                    </button>
                    <button className={`px-3 font-semibold ${activeTab === 'dinner' ? 'px-3 py-1 bg-secondary rounded-md whitespace-nowrap' : ''}`} onClick={() => setActiveTab('dinner')}>
                        Dinner
                    </button>
                </div>
            </div>
